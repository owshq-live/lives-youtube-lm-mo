...
if __name__ == '__main__':
    ...
    spark = SparkSession \
        ...
        .config("spark.hadoop.fs.s3a.secret.key", "YourScretKey") \
        ...
        .config("spark.sql.catalog.owshq.uri", "thrift://localhost:9083") \
        .config("spark.hadoop.fs.s3a.impl", "org.apache.hadoop.fs.s3a.S3AFileSystem") \
        .config("spark.sql.catalog.owshq.io-impl", "org.apache.iceberg.aws.s3.S3FileIO") \
        .config("spark.sql.catalog.owshq.warehouse", "s3://warehouse/metastore") \
        .config("spark.sql.catalog.owshq.s3.endpoint", "http://minio-ip") \
        .config("spark.sql.defaultCatalog", "owshq") \
        .getOrCreate()
    ...
    # get_device_file = "/Users/luanmorenomaciel/BitBucket/owshq-svc-spark/etl-enriched-users-analysis/files/device_2022_6_10_0_0_31.json"
    # get_subscription_file = "/Users/luanmorenomaciel/BitBucket/owshq-svc-spark/etl-enriched-users-analysis/files/subscription_2022_6_10_0_0_28.json"
    ...
    create_db = spark.sql("""CREATE DATABASE IF NOT EXISTS default;""")
    create_db.show()

    # create iceberg table
    create_device_table_iceberg = spark.sql("""
        CREATE TABLE IF NOT EXISTS owshq.default.device
        (
            build_number int,
    ...
    create_subscription_table_iceberg = spark.sql("""
          CREATE TABLE IF NOT EXISTS owshq.default.subscription
          (
              dt_current_timestamp long,
              id int,
    ...
    spark.table("vw_device").writeTo("owshq.default.device").append()
    spark.table("vw_subscription").writeTo("owshq.default.subscription").append()

    # select data & query data
    query_device = spark.table("owshq.default.device")
    query_subscription = spark.table("owshq.default.subscription")
    ...
    create_plans_table_iceberg = spark.sql("""
        CREATE TABLE IF NOT EXISTS owshq.default.plans
        (
            plan string,
            price decimal(4,2),
            importance string,
    ...
    spark.table("vw_plans").writeTo("owshq.default.plans").append()

    # get table's info
    get_info_device = spark.sql("""DESCRIBE FORMATTED owshq.default.device""")
    get_info_subscription = spark.sql("""DESCRIBE FORMATTED owshq.default.subscription""")
    get_info_plans = spark.sql("""DESCRIBE FORMATTED owshq.default.plans""")
